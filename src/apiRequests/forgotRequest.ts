...
export const getUserByToken = async (
  ...
  try {
    let res = await axiosInstance.get<ResponseGetUserFromToken>(
...
export const changPassword = async (
  ...
  try {
    let res = await axiosInstance.post("/user/change-password", data);
...
export const sendMailForgotPassword = async (
  ...
  try {
    let res = await axiosInstance.post<ResponseGetMailToken>(
      "/user/forgot-password",
      {
        usernameOrEmail,
      },
...
export const forgotPasswords = async (
  ...
  try {
    let res = await axiosInstance.get<ResponseGetEmail>("/user", {
      params: {
    ...
    navigate("/verify");
    sendMailForgotPassword(res.data.responseData, dispatch, navigate);
  } catch (error: any) {
    if (error.response) {
      if (error.response.status === 400) {
        dispatch(sendMailRecoveryFailed(error.response.data.message));
